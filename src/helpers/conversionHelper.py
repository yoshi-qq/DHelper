...
def toItem(_id: str, jsonItem: JsonItem) -> Item:
    ...
    damageDict = (
        {
            ...
            "damageType": (
                to_enum(DamageType, damage.get("damageType"))
                if damage.get("damageType")
                else None
            ),
        }
        if damage
    ...
    return Item(
        ...
        attributes=(
            [to_enum(AttributeType, a) for a in (jsonItem.get("attributes") or [])]
            if isinstance(jsonItem.get("attributes"), list)
            else []
        ),
...
def toSpell(_id: str, jsonSpell: JsonSpell) -> Spell:
    ...
    damage = (
        Damage(
            ...
            dmg.get("bonus", 0),
            (
                to_enum(DamageType, dmg.get("damageType"))
                if dmg.get("damageType")
    ...
    return Spell(
        ...
        casterClasses=[
            to_enum(CasterClassType, c) for c in jsonSpell.get("casterClasses", [])
        ],
        ...
        savingThrow=(
            to_enum(SavingThrowType, jsonSpell.get("savingThrow"))
            if jsonSpell.get("savingThrow")
            else None
        ),
        areaOfEffect=(
            to_enum(TargetType, jsonSpell.get("areaOfEffect"))
            if jsonSpell.get("areaOfEffect")
            else None
        ),
    )
